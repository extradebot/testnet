@php
    ...
    {
        $stake = Staking::where('user_id', user('id'))
        ...
        <div class="w-full flex justify-center">
            <div class="w-11/12 rounded-md bg-[#0e1726] p-2 md:p-4">
                <div class="flex justify-between items-center">
                    ...
                    <div>
                        <a href="@if (url()->previous() == route('login')) {{ route('user.dashboard') }} @else {{ url()->previous() }} @endif"
                        ...
                        <tbody width="100%">
                            @foreach ($coins as $coin)
                                <tr>
                                    ...
                                    <td><img src="{{ asset('assets/imgs/' . $coin->icon) }}" alt="{{ $coin->coin }}"
                                            width="50px" class="rounded-full"></td>
                                    ...
                                    <td class="inline-flex space-x-3 md:space-x-5 @if (getCurrentStake($coin->id) <= $coin->max_stake) stake-button @endif"
                                        data-type="open" data-coin_id="{{ $coin->id }}"
                                        data-coin="{{ $coin->coin }}" data-symbol="{{ $coin->symbol }}"
                                        data-duration="{{ $coin->duration }}" data-total="{{ $coin->total }}"
                                        data-staked="{{ $coin->staked }}" data-apr="{{ $coin->apr }}"
                                        data-daily_return="{{ $coin->daily_return }}"
                                        data-min_stake="{{ $coin->min_stake }}" data-max_stake="{{ $coin->max_stake }}"
                                        data-price="{{ $coin->price }}">
                                        <a role="button" title="Stake {{ $coin->coin }}">
                                            @if (getCurrentStake($coin->id) <= $coin->max_stake)
                                                <span
                                                    class="bg-green-500 p-2 rounded text-white">{{ ct('Stake Now') }}</span>
                                            @else
                    ...
                    <div class="w-full p-6 md:p-10 flex justify-center">
                        <div
                            ...
                            <div>
                                ...
                                {{ ct('There are no user coins found.') }}
                            </div>
                        </div>
                    </div>
                @endif
