@php
    ...
    {
        $check = \Modules\CryptoTrading\Entities\TradingBotActivation::where('bot_id', $bot_id)
...
<div class="bots-popup hidden">
    <div class="bots-inner">
        ...
        @foreach ($bots as $bot)
            <div class="bot">
                <div class=""><img src="{{ asset('assets/imgs/' . $bot->icon) }}" alt="bot" width="35px"
                        class="rounded rounded-circle"></div>
                <div class="">{{ $bot->name }}</div>
                <div class="">{{ formatAmount($bot->price) }}</div>
                <div class="px-3">
                    @if (isActivated($bot->id))
                        <span class="is_activated btn btn-success rounded" data-bot_id="{{ $bot->id }}"
                            data-bot_name="{{ $bot->name }}">{{ ct('Select') }}</span>
                    @else
                        <span class="is_not_activated btn btn-primary rounded"
                            data-bot_id="{{ $bot->id }}">{{ ct('Activate') }}</span>
                    @endif
                </div>
...
<div class="form-popup hidden">
    <div class="bots-inner">
        <form action="{{ route('user.trading.trade.bot-activate') }}" method="post">
            ...
            <div class="bot">
                <button type="submit"
                    class="form-control btn btn-primary rounded bg-primary">{{ ct('Activate Now') }}</button>
            </div>
        </form>
    </div>
