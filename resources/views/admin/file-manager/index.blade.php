    ...
    if (!function_exists('formatImageLink')) {
        function formatImageLink($item)
        {
            ...
            if (str()->contains($n_item, '/public/assets')) {
                ...
                $n_item = $n_item[1];
                $image_link = asset('assets' . $n_item);
            } else {
                //private image
        ...
        <div class="w-full flex justify-center">
            <div class="w-11/12 rounded-sm bg-[#0e1726] p-1 md:p-4">
                <div class="file-upload-panel @if (!request()->has('folder')) hidden @endif">
                    <form class="mt-5" action="{{ route('admin.file-manager.upload') }}" method="POST"
                    ...
                    <div class="flex justify-end space-x-2">
                        ...
                        <div class="manager-nav-panel-btn">
                            <a href="{{ route('admin.file-manager.index') }}" title="Back to file manager"
                        ...
                        <div class="manager-nav-panel-btn">
                            <a href="{{ request()->return_to }}" title="Back"
                                class="flex justify-start items-center space-x-1 text-xs md:text-sm text-[#d1d5db] text-center px-5 py-2 my-2 bg-[#1b2e4b] hover:bg-gray-700 rounded-md">
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                ...
                                <tbody width="100%">
                                    @foreach ($root_folders as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <span data-folder="{{ urlencode($item) }}"
                                                    class="folder-btn flex justify-start">
                                                    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"
                                                        xmlns="http://www.w3.org/2000/svg">
                                                        <path fill-rule="evenodd"
                                    ...
                                    @foreach ($root_files as $item)
                                        <tr id="{{ 'file-' . $loop->iteration }}">
                                            ...
                                            <td class="inline-flex space-x-3 md:space-x-5">
                                                <a
                                                    href="{{ route('admin.file-manager.download', ['file' => urlencode($item)]) }}">
                                                    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"
                                                        xmlns="http://www.w3.org/2000/svg">
                                                        <path fill-rule="evenodd"
                                                ...
                                                @if (getFileType($item) == 'code')
                                                    <a href="{{ 'vscode://file/' . urlencode($item) }}" target="_blank">
                                                        <img src="{{ asset('assets/imgs/vscode.svg') }}" class="h-5 w-5"
                                                            alt="">
                                                    </a>
                                                    <a href="{{ route('admin.file-manager.editor', ['file' => urlencode($item)]) }}"
                                                        target="_blank">
                                                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <path fill-rule="evenodd"
                                                ...
                                                @elseif (getFileType($item) == 'image')
                                                    ...
                                                    <button type="button" class="file_delete_btn"
                                                        data-hide="{{ 'file-' . $loop->iteration }}"
                                                        data-file="{{ urlencode($item) }}">
...
@section('script')
    <script>
        $('.folder-btn').on('click', function() {
            ...
            var url = "{{ route('admin.file-manager.index') }}?folder=" + folder +
                "&return_to={{ urlencode(request()->fullUrl()) }}";
